...
CLevel::Initialise(int _iWidth, int _iHeight)
{
    ...
    VALIDATE(m_pBullet->Initialise(m_iWidth / 2.0f, m_iHeight / 2.0f, fBallVelX, fBallVelY));
    ...
    m_pShip->SetY(_iHeight - ( 1.5f * m_pShip->GetHeight()));

    const int kiNumInavders = 36;
    ...
    {
        CInvaders* pBrick = new CInvaders();
        ...
        m_vecInvaders.push_back(pBrick);
    }
    ...
    for (unsigned int i = 0; i < m_vecInvaders.size(); ++i)
    {
        if (!m_vecInvaders[i]->IsHit())
        {
                ...
                (fBallY - fBallR < fBrickY + fBrickH / 2))
            {
                ...
                SetInvadersRemaining(GetBricksRemaining() - 1);
            }
        }
    }
}
